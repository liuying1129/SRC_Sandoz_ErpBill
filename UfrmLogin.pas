...
begin
  ...
  begin
    ...
    ExecSQLCmd(LisConn,'insert into AppVisit (IP,UserName,ActionName,ActionTime,Reserve4,ComputerName) values ('''+gsLocalIP+''','''+LabeledEdit4.Text+''',''登录'',getdate(),''登录失败,密码错误'','''+gsLocalName+''')');
  end
  else   //成功登录
  begin
    close;
    ...
    ExecSQLCmd(LisConn,'insert into AppVisit (IP,UserName,ActionName,ActionTime,Reserve4,ComputerName) values ('''+gsLocalIP+''','''+operator_name+''',''登录'',getdate(),''登录成功'','''+gsLocalName+''')');
  end;
end;
